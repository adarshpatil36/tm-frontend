...
export const getAllTickets = () => {
  return new Promise(async (resolve, reject) => {
    try {
      const result = await axios.get("http://localhost:3001/v1/ticket/getAll", {
        headers: {
...
export const getTicketByDesc = (speechToText) => {
  var result = speechToText === null;
  console.log("ticket api", speechToText);
  return new Promise(async (resolve, reject) => {
    try {
      const result = await axios.post(
        "http://localhost:3001/v1/ticket/search",
        { speechToText },
        {
          headers: {
            Authorization: sessionStorage.getItem("accessJWT"),
